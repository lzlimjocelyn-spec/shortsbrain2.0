...
  if (mktKey && metaMap[mktKey] && metaMap[mktKey][campKey]) {
    return metaMap[mktKey][campKey];
  }
  if (metaMap[campKey]) return metaMap[campKey];
  return {};
};
...
    
    // Attempt to find start/end dates from the data CSV itself if meta is missing
...
        // If we are processing market-specific hub or AO data, and no structural meta exists, skip the campaign
        if (marketContext && (!meta || Object.keys(meta).length === 0)) {
          return;
        }
        
        // Use data-file dates as fallback if meta is empty
...
      ))}
    </div>
    <button type="button" onClick={() => handleAllToggle()} className={`px-6 py-2.5 rounded-md text-[10px] font-bold tracking-widest uppercase border transition-all cursor-pointer ${activeMetrics.length === M_TYPES.length ? 'bg-white text-black border-white' : 'bg-transparent text-[#808080] border-[#3a3a3a] hover:border-[#808080]'}`}>
...
            // Robust calculation for live duration
...
    const diffTime = Math.ceil((endQ - new Date(latestDate)) / (1000 * 60 * 60 * 24));
...
        if (!camp.meta || !camp.meta.market || !eq(camp.meta.market, market)) return;

        const metrics = camp.metrics?.['DAU-SCT'] || {};
        const isNeg = (g, a) => (metrics[g]?.[a]?.v || 0) < -0.0001;
        const isPosSig = (g, a) => (metrics[g]?.[a]?.sig || 0) === 1;
...
        const aoExempt = ['SHELF', 'SSC', 'UTS', 'MVR', 'UTS SFV'];
        const isAO = aoExempt.some(ex => superClean(camp.country).includes(superClean(ex)) || (camp.meta?.subTab && superClean(camp.meta.subTab).includes(superClean(ex))));
        const aoPauseSuffix = "pause the current trend and wait for 5-7d and decide to rollout next trend or not";

        if (isNeg('female', 'total') && isNeg('male', 'total')) {
          const node = metrics.total?.total || { v: 0, sig: 0 };
          const justification = isAO 
            ? `${mKey} ${camp.country} - Pause GGenPop given both genders trending negative (${node.v.toFixed(2)}%) - ${aoPauseSuffix}`
            : `${mKey} ${camp.country} - Pause GGenPop given both genders trending negative (${node.v.toFixed(2)}%)`;
          
          tableData.push({ 
            id: `CAMP_${market}_${ci}_PAUSE_TOTAL`, 
            country: mKey, campaign: camp.country, age: "GenPop", gender: "GenPop", 
            recommendation: "PAUSE", justification 
          });
          return; 
        }

        if (daysNum < 14 && daysNum >= 0) {
          tableData.push({ 
            id: `CAMP_${market}_${ci}_MAINTAIN`, 
            country: mKey, campaign: camp.country, age: "GenPop", gender: "GenPop", 
            recommendation: "MAINTAIN", justification: `${mKey} ${camp.country} - Learning phase stage: live for ${daysNum} days (<14d).` 
          });
          return;
        }

        const gpNode = metrics.total?.total || { v: 0, sig: 0 };
        if (gpNode.sig === 1 && gpNode.v > 0.001) {
          tableData.push({
            id: `CAMP_${market}_${ci}_SCALE_GP`,
            country: mKey, campaign: camp.country, age: "GenPop", gender: "GenPop",
            recommendation: "SCALE",
            justification: `${mKey} ${camp.country} - Scale GGenPop: Stat-sig positive lift (+${gpNode.v.toFixed(2)}%) observed.`
          });
          return;
        }

        const getDirectivesForGender = (genderKey) => {
          const res = [];
          const genderInit = genderKey === 'total' ? 'G' : genderKey.charAt(0).toUpperCase();
          let age_dirs = [];
          let age1834_cons = false;
          if (isNeg(genderKey, '18-24') && isNeg(genderKey, '25-34')) age1834_cons = true;
          if (age1834_cons && isNeg(genderKey, '35+')) age_dirs.push('total');
          else {
            if (age1834_cons) age_dirs.push('18-34');
            else {
              if (isNeg(genderKey, '18-24')) age_dirs.push('18-24');
              if (isNeg(genderKey, '25-34')) age_dirs.push('25-34');
            }
            if (isNeg(genderKey, '35+')) age_dirs.push('35+');
          }
          age_dirs.forEach(age => {
            const node = metrics[genderKey]?.[age] || { v: 0, sig: 0 };
            const sigLabel = node.sig === -1 ? "stat sig negative" : "neutral negative";
            const ageTag = age === 'total' ? 'GenPop' : age;
            const justification = isAO
              ? `${mKey} ${camp.country} - Pause ${genderInit}${ageTag} given ${sigLabel} (${node.v.toFixed(2)}%) - ${aoPauseSuffix}`
              : `${mKey} ${camp.country} - Pause ${genderInit}${ageTag} given ${sigLabel} (${node.v.toFixed(2)}%)`;
            res.push({ g: genderKey, a: age, recommendation: "PAUSE", justification });
          });
          return res;
        };

        const maleDirectives = getDirectivesForGender('male');
        const femaleDirectives = getDirectivesForGender('female');

        [...maleDirectives, ...femaleDirectives].forEach((d, di) => {
          tableData.push({ ...d, id: `CAMP_${market}_${ci}_D_${di}`, country: mKey, campaign: camp.country, age: d.a === 'total' ? 'GenPop' : d.a, gender: d.g === 'total' ? 'GenPop' : d.g.toUpperCase() });
        });
        
        DEMO_PERMS.filter(d => d.g !== 'total').forEach(demo => {
          if (isPosSig(demo.g, demo.a)) {
            const val = metrics[demo.g]?.[demo.a]?.v || 0;
            const gChar = demo.g.charAt(0).toUpperCase();
            const aTag = demo.a === 'total' ? 'GenPop' : demo.a;
            tableData.push({
              id: `CAMP_${market}_${ci}_SCALE_${demo.g}_${demo.a}`,
              country: mKey, campaign: camp.country, age: demo.a === 'total' ? 'GenPop' : demo.a, gender: demo.g.toUpperCase(),
              recommendation: "SCALE", justification: `${mKey} ${camp.country} - Scale ${gChar}${aTag} given stat-sig positive lift (+${val.toFixed(2)}%).`
            });
          }
        });
      });
...
  const handleCopyRow = async (row) => { const text = `${row.country}\t${row.campaign}\t${row.age}\t${row.gender}\t${row.recommendation}\t${row.justification}`; await copyToClipboard(text); showFeedback("Row Copied to Clipboard"); };
  const handleCopyAll = async () => { if (recommendationRows.length === 0) return; const headers = "Market\tEntity\tAge\tGender\tDirective\tJustification"; const body = recommendationRows.map(r => `${r.country}\t${r.campaign}\t${r.age}\t${r.gender}\t${r.recommendation}\t${r.justification}`).join('\n'); await copyToClipboard(`${headers}\n${body}`); showFeedback("Full Matrix Copied for Sheets"); };
  const handleRestore = () => { setDeletedRowIds(new Set()); setEditedRows({}); setManualPointers([]); showFeedback("Matrix Restored to Baseline"); };
  const handleAddNewManual = () => { if (!newManualForm.campaign) return; const id = `MANUAL_${Date.now()}`; setManualPointers(p => [...p, { ...newManualForm, id, statusType: newManualForm.recommendation === 'PAUSE' ? 'danger' : (newManualForm.recommendation === 'SCALE' ? 'success' : 'warning') }]); setIsAddingManual(false); setNewManualForm({ country: 'APAC', campaign: '', age: 'GenPop', gender: 'GenPop', recommendation: 'MAINTAIN', justification: '' }); };
...
                {!stat.isOffline && <BarChart3 className={`w-5 h-5 ${cfg.color}`} />}
              </div>
...
            <div className="p-3 bg-[#1a1a1a] rounded-lg border border-[#3a3a3a] shadow-inner"><Lightbulb className="w-6 h-6 text-amber-400" /></div>
            <div><h2 className="text-2xl font-bold text-white uppercase">Strategic Guidance</h2><p className="text-[#808080] text-xs uppercase tracking-widest mt-1 font-medium">Data-Driven Directives & Manual Overrides</p></div>
          </div>
          <div className="flex flex-wrap gap-3">
            <button type="button" onClick={() => setIsAddingManual(true)} className="flex items-center gap-2 bg-[#FF0000] text-white px-5 py-2.5 rounded-lg text-[10px] font-bold uppercase hover:bg-red-500 transition-all shadow-lg active:scale-95"><Plus className="w-4 h-4" /> Add Pointer</button>
            <button type="button" onClick={() => handleCopyAll()} className="flex items-center gap-2 bg-white text-black px-5 py-2.5 rounded-lg text-[10px] font-bold uppercase hover:bg-[#e0e0e0] transition-all shadow-lg active:scale-95"><Copy className="w-4 h-4" /> Copy All for Sheets</button>
            <button type="button" onClick={() => handleRestore()} className="flex items-center gap-2 bg-[#1a1a1a] text-[#808080] px-5 py-2.5 rounded-lg text-[10px] font-bold uppercase hover:text-white transition-all border border-[#3a3a3a] active:scale-95"><RotateCcw className="w-4 h-4" /> Restore Defaults</button>
          </div>
        </div>
...
                <select className="w-full bg-black border border-[#3a3a3a] rounded-md p-2.5 text-[11px] font-bold outline-none text-white cursor-pointer" value={newManualForm.recommendation} onChange={e => setNewManualForm(p => ({...p, recommendation: e.target.value}))}>
                  <option value="MAINTAIN">MAINTAIN</option><option value="SCALE">SCALE</option><option value="PAUSE">PAUSE</option>
                </select>
              </div>
            </div>
            <textarea className="w-full h-24 bg-black border border-[#3a3a3a] rounded-lg p-3 text-[11px] text-[#b0b0b0] outline-none focus:border-[#FF0000] mb-4 resize-none" placeholder="Strategic justification context..." value={newManualForm.justification} onChange={e => setNewManualForm(p => ({...p, justification: e.target.value}))} />
            <div className="flex justify-end gap-3">
              <button type="button" onClick={() => setIsAddingManual(false)} className="bg-[#1a1a1a] text-[#808080] px-5 py-2.5 rounded-lg hover:text-white font-bold text-[10px] uppercase transition-all border border-[#3a3a3a]">Cancel</button>
              <button type="button" onClick={() => handleAddNewManual()} className="bg-emerald-600 text-white px-6 py-2.5 rounded-lg hover:bg-emerald-500 font-bold text-[10px] uppercase transition-all shadow-lg">Confirm Pointer</button>
            </div>
          </div>
        )}
...
                    {editingId === row.id ? (
                      <select className="bg-black/60 border border-white/10 rounded-lg p-2 text-[10px] outline-none text-white" value={editedRows[row.id]?.recommendation || row.recommendation} onChange={e => setEditedRows(p => ({...p, [row.id]: {...(p[row.id] || row), recommendation: e.target.value}}))}>
                        <option value="MAINTAIN">MAINTAIN</option><option value="SCALE">SCALE</option><option value="PAUSE">PAUSE</option>
                      </select>
                    ) : (
...
                    {editingId === row.id ? (
                      <textarea className="w-full bg-black/60 border border-white/10 rounded-lg p-2 text-[10px] outline-none min-h-[60px] resize-none" value={editedRows[row.id]?.justification || row.justification} onChange={e => setEditedRows(p => ({...p, [row.id]: {...(p[row.id] || row), justification: e.target.value}}))} />
                    ) : row.justification}
                  </td>
...
                      {editingId === row.id ? (
                        <button type="button" onClick={() => handleSetEditingId(null)} className="p-2.5 rounded-xl bg-emerald-600 text-white hover:bg-emerald-500 shadow-lg"><Save className="w-4 h-4" /></button>
                      ) : (
                        <button type="button" onClick={() => handleSetEditingId(row.id)} title="Edit Row" className="p-2.5 rounded-xl bg-white/5 text-slate-400 hover:text-white hover:bg-white/10 transition-all"><Edit2 className="w-4 h-4" /></button>
                      )}
                      <button type="button" onClick={() => handleCopyRow(row)} title="Copy Row" className="p-2.5 rounded-xl bg-white/5 text-slate-400 hover:text-emerald-400 hover:bg-white/10 transition-all"><Copy className="w-4 h-4" /></button>
                      <button type="button" onClick={() => setDeletedRowIds(prev => new Set([...prev, row.id]))} title="Delete Pointer" className="p-2.5 rounded-xl bg-white/5 text-slate-400 hover:text-red-500 hover:bg-white/10 transition-all"><Trash2 className="w-4 h-4" /></button>
                    </div>
                  </td>
                </tr>
              ))}
...
             <div className="p-20 text-center text-[#555] font-bold uppercase tracking-widest text-[10px]">Matrix Empty - Run Analysis to Populate</div>
          )}
        </div>
      </div>
    </div>
...
    <div className={`p-6 rounded-lg border ${type === 'pct' ? 'border-amber-500/30 bg-[#1a1500]' : 'border-blue-500/30 bg-[#0a0a1a]'} mb-6 transition-all shadow-xl`}>
...
        <HubRow type="pct" title="Ratio-Based Analysis" tag="Relative Lift Streams (%)" icon={TrendingUp} />
        <HubRow type="abs" title="Volume-Based Analysis" tag="Discrete Delta Streams (Delta)" icon={Binary} />
...
    const appId = typeof __app_id !== 'undefined' ? __app_id : 'shorts-brain-2-0';
...
    const q = collection(db, 'artifacts', appId, 'public', 'data', 'snapshots');
    const unsubscribe = onSnapshot(q, (snapshot) => {
      const docs = snapshot.docs.map(d => ({...d.data(), id: d.id}));
      setMemoryIndex(docs.sort((a,b) => b.timestamp - a.timestamp));
    }, (err) => {
...
  const loadHistoricalWeek = async (weekId) => {
    const snap = memoryIndex.find(s => s.weekId === weekId);
    if (!snap) return;
...
      setHistoricalSnapshots(prev => [...prev.filter(h => h.weekId !== weekId), snap]);
      setIsAnalyzed(true);
    } catch (err) {
      console.error('Load historical week error:', err);
    } finally {
      setIsLoadingMemory(false);
    }
  };

  const deleteSnapshotFromCloud = async (id) => {
    if (!user) return;
    const firebaseConfigStr = typeof __firebase_config !== 'undefined' ? __firebase_config : '{}';
    const firebaseConfig = JSON.parse(firebaseConfigStr);
    const app = initializeApp(firebaseConfig);
...
    const appId = typeof __app_id !== 'undefined' ? __app_id : 'shorts-brain-2-0';
    try {
      await deleteDoc(doc(db, 'artifacts', appId, 'public', 'data', 'snapshots', id));
    } catch (err) {
...
              const metaObj = { market: resolvedMkt, tab: cleanStr(cols[tabIdx]), subTab: cleanStr(cols[subTabIdx]), subSubTab: cleanStr(cols[subSubTabIdx]), campaignStartDate: cleanStr(cols[startIdx]), campaignEndDate: cleanStr(cols[endIdx]), optimisationEndDate: cleanStr(cols[optIdx]) };
              
              const mktKey = superClean(resolvedMkt);
              const campKey = superClean(name);
              if (!metaLookup[mktKey]) metaLookup[mktKey] = {};
              metaLookup[mktKey][campKey] = metaObj;
            }
          });
        }
      }
...
        const isGlobalHub = defaultMarket === 'APAC';

        Object.values(parsedMap).forEach(row => {
          const meta = row.meta || {};
          
          if (!isGlobalHub && meta.market && !eq(meta.market, defaultMarket)) return;

          let rawTab = forceTab || cleanStr(meta.tab);
          
          const matchedChild = rawTab ? CAMPAIGN_CHILDREN.find(child => eq(child.id, rawTab) || eq(child.label, rawTab)) : null;
          const tabKey = matchedChild ? matchedChild.id : null;
...
                if (isGlobalHub) {
                  row.metrics[m][g][a].isPaused = false;
                  return;
                }
                if (hasEnded) {
...
                  const checkKeys = [[g, sanA], parentA ? [g, parentA] : null, [g, 'total'], ['total', sanA], parentA ? ['total', parentA] : null, ['total', 'total']].filter(Boolean);
...
            }
          }
          streamGData = parseCSVData(text, {}, metaLookup, ['Country', 'Market', 'Campaign'], isAbs, null);
        }
        const mHubParsed = {};
        for (const m of MARKET_SEGMENTS) { if (stream.countryHB[m]) { const text = await readFile(stream.countryHB[m]); mHubParsed[m] = parseCSVData(text, {}, metaLookup, undefined, isAbs, m); } }
        const alwaysOnParsed = {};
        for (const cat of AO_CATEGORIES) { if (stream.alwaysOn[cat]) { const text = await readFile(stream.alwaysOn[cat]); alwaysOnParsed[cat] = parseCSVData(text, {}, metaLookup, undefined, isAbs, 'India'); } }
...
          else { M_TYPES.forEach(m => { GENDERS_KEYS.forEach(g => { AGE_BUCKETS.forEach(a => { merged[key].metrics[m][g][a].abs = absMap[key].metrics[m][g][a].abs; }); }); }); }
        });
        return merged;
      };
...
        
        // Strict filter: only keep rows that have valid metadata (prevents showing campaigns not present in meta)
        const mFilteredRows = Object.values(mMerged).filter(row => row.meta && Object.keys(row.meta).length > 0);
        regionalMerged[m] = mFilteredRows;
        
        routeData(mMerged, m);
      });
      AO_CATEGORIES.forEach((cat) => {
        const mergedAO = mergeResults(pctResults.alwaysOnParsed[cat] || {}, absResults.alwaysOnParsed[cat] || {});
        // Route AO data to AlwaysOn tab
...
      setIsAnalyzed(true);
      await saveSnapshotToCloud({ reportingDate: detectedGlobalDate, globalData: globalDataArr, regionalData: regionalMerged, campaignHubData: finalStructData });
...
      }
    }
  }, [activeTab, tabMarketFilter, campaignHubData, getSubTabs, getSubSubTabs]);
...
            return (
              <button key={item.id} type="button" onClick={() => { if (item.id === 'Upload') setIsAnalyzed(false); else setActiveTab(item.id); }} className={`w-full flex items-center gap-3 p-3 rounded-xl transition-all group relative cursor-pointer ${activeTab === item.id ? 'bg-[#FF0000]/10 text-[#FF0000] border border-[#FF0000]/20' : 'text-[#808080] hover:bg-white/5 hover:text-white border border-transparent'}`}>
...
                {!isSidebarOpen && <div className="absolute left-full ml-2 px-2 py-1 bg-[#1a1a1a] border border-[#3a3a3a] text-white text-[10px] uppercase font-bold rounded opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap z-[100]">{item.label}</div>}
              </button>
            );
          })}
          <div className="my-4 border-t border-[#222]"></div>
          <button type="button" onClick={() => setIsCampaignTypeExpanded(!isCampaignTypeExpanded)} className="w-full flex items-center justify-between p-3 rounded-xl text-[#808080] hover:text-white cursor-pointer border border-transparent">
            <div className="flex items-center gap-3"><FolderKanban className="w-5 h-5 shrink-0" />{isSidebarOpen && <span className="text-[11px] font-bold uppercase tracking-wider">Campaign Hub</span>}</div>
...
                    <ChildIcon className="w-4 h-4 shrink-0" /><span className="text-[10px] font-bold uppercase">{child.label}</span>
                  </button>
                );
              })}
            </div>
...
                <span className="text-[10px] font-bold uppercase text-[#444] tracking-widest">Memory</span>
                {memoryStatus === 'saving' && <Loader2 className="w-3 h-3 text-amber-500 animate-spin ml-auto" />}
                {memoryStatus === 'saved' && <CheckCircle2 className="w-3 h-3 text-emerald-500 ml-auto" />}
                {memoryStatus === 'error' && <AlertCircle className="w-3 h-3 text-red-500 ml-auto" />}
              </div>
              <div className="space-y-1 max-h-[300px] overflow-y-auto pr-1">
                {memoryIndex.length === 0 ? (
                  <p className="text-[9px] text-[#333] px-3 py-4 text-center font-bold uppercase italic">No Snapshots</p>
                ) : (
                  memoryIndex.map(snap => (
                    <div key={snap.id} className={`w-full flex items-center gap-2 px-3 py-2 rounded-lg text-left transition-all group ${historicalSnapshots.some(h => h.weekId === snap.weekId) ? 'bg-emerald-500/10 text-emerald-400 border border-emerald-500/20' : 'text-[#666] hover:bg-white/5 hover:text-white border border-transparent'}`}>
                      <button type="button" onClick={() => loadHistoricalWeek(snap.weekId)} disabled={isLoadingMemory} className="flex-1 flex items-center gap-2 text-left cursor-pointer min-w-0">
                        <Clock className="w-3.5 h-3.5 shrink-0" />
                        <div className="flex-1 min-w-0">
                          <span className="text-[10px] font-bold block">{snap.weekId}</span>
                          <span className="text-[8px] opacity-60 font-mono tracking-tighter">{snap.reportingDate || 'Generic Date'}</span>
                        </div>
                      </button>
                      <button type="button" onClick={() => deleteSnapshotFromCloud(snap.id)} className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity p-1 hover:bg-white/10 rounded">
...
          )}
        </nav>
        <button type="button" onClick={() => setIsSidebarOpen(!isSidebarOpen)} className="p-6 border-t border-[#2a2a2a] text-[#555] hover:text-white flex items-center justify-center transition-colors">
...
          </div>
          <div className="flex items-center gap-4">
             <button type="button" className="bg-white/5 border border-white/10 text-[#888] px-5 py-2 rounded-lg text-[10px] font-bold uppercase hover:text-white hover:border-white/20 transition-all flex items-center gap-2 active:scale-95"><Filter className="w-3.5 h-3.5" /> Filter Matrix</button>
             <button type="button" className="bg-white text-black px-6 py-2.5 rounded-lg text-[10px] font-bold uppercase hover:bg-[#e0e0e0] transition-all shadow-xl flex items-center gap-2 active:scale-95"><Download className="w-3.5 h-3.5" /> Export Data</button>
          </div>
        </header>
...
                  <select value={activeMarketSubTab} onChange={e => setActiveMarketSubTab(e.target.value)} className="bg-transparent text-white font-bold uppercase outline-none cursor-pointer pr-8">
...
                  const campaigns = regionalData[activeMarketSubTab] || [];
...
                  <div className="flex items-center gap-4 p-4 bg-[#111] rounded-xl border border-[#2a2a2a] shadow-lg">
                    <MapPin className="w-6 h-6 text-red-600" />
...
                    <div className="flex gap-2 p-1 bg-[#111] rounded-xl border border-[#2a2a2a] shadow-lg overflow-x-auto max-w-full no-scrollbar">
                      {getSubTabs(activeTab, tabMarketFilter[activeTab]).map(s => (
                        <button key={s} type="button" onClick={() => setSubTabFilter(p => ({ ...p, [activeTab]: s }))} className={`px-5 py-2.5 rounded-lg text-[9px] font-bold uppercase transition-all duration-500 cursor-pointer whitespace-nowrap ${subTabFilter[activeTab] === s ? 'bg-[#FF0000] text-white shadow-lg shadow-red-500/20' : 'text-[#555] hover:text-white'}`}>{s}</button>
                      ))}
                    </div>
                  )}
                </div>
                {getSubSubTabs(activeTab, tabMarketFilter[activeTab], subTabFilter[activeTab]).length > 0 && (
                  <div className="flex gap-2 p-1.5 bg-[#1a1a1a] rounded-xl border border-white/5 shadow-inner overflow-x-auto max-w-full no-scrollbar">
                    {getSubSubTabs(activeTab, tabMarketFilter[activeTab], subTabFilter[activeTab]).map(ss => (
                      <button key={ss} type="button" onClick={() => setSubSubTabFilter(p => ({ ...p, [activeTab]: ss }))} className={`px-4 py-2 rounded-lg text-[8px] font-bold uppercase transition-all cursor-pointer whitespace-nowrap ${subSubTabFilter[activeTab] === ss ? 'bg-white text-black' : 'text-[#444] hover:text-white'}`}>{ss}</button>
                    ))}
                  </div>
                )}
              </div>
...
                  const rows = ss === 'Default' || !ss ? (path ? Object.values(path).flat() : []) : (path?.[ss] || []); 

                  // Sort Always-On Trends by Trend Start Date (Ascending)
                  if (activeTab === 'AlwaysOn') {
                    return [...rows].sort((a, b) => {
                      const dateA = a.campaignStartDate || '9999-12-31';
                      const dateB = b.campaignStartDate || '9999-12-31';
...
      </div>
      <style>{`
        body { background-color: #000000; color: #e0e0e0; margin: 0; -webkit-font-smoothing: antialiased; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, sans-serif; overflow: hidden; }
